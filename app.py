...
def answer_with_docs(question: str, llm: ChatOllama) -> str:
    docs = load_docs()
    selected_docs = keyword_retriever(question, docs, top_k=2)

    context = "\n\n".join(selected_docs) if selected_docs else "No relevant documents found."

    prompt = ChatPromptTemplate.from_template("""
You are an assistant for a MOBILE retail commerce app demo.

Rules:
- Answer ONLY with the information contained in the provided context.
- Do NOT invent policies, features, procedures, prices, or assumptions.
- If the answer is not explicitly present in the context, reply exactly:
  "La POC todavía no contiene esa información."
- Answer in Spanish.
...
if get_current_turns() < MAX_TURNS and question:
    ...
    # -------------------------------------------------
        ...
        if farewell_answer:
            ...
            llm_precise = ChatOllama(model=model_name, temperature=0.0)
            llm_creative = ChatOllama(model=model_name, temperature=0.2)
            ...
            if route == "sql":
                ...
                try:
                    response_text = answer_with_docs(question, llm_creative)
                except Exception:
                    response_text = "El modelo local aún no está disponible."
